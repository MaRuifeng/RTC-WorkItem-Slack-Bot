...
function load_env(name, desc) {
    ...
    if (name == "RTC_PASS" && ("" == value || undefined === value)){
...
var BUILD = load_env("WIBOT_VERSION", "The build version of the wibot container");
...
//        console.log(wi);
        var titleText = "Work Item " + wi["dc:identifier"] + ": " + html2text.fromString(wi["dc:title"]);

        console.log(titleText);

        var statusLine = '';

        if (wi["rtc_cm:state"] && wi["rtc_cm:state"]["rdf:resource"]) {
            var stateUri = wi["rtc_cm:state"]["rdf:resource"];
            wiFetcher.resolveResource(stateUri).then(function(stateResource) {
                statusLine += '_State: *' + stateResource["dc:title"] + '*_. ';
                ...
                if (wi["rtc_cm:ownedBy"] && wi["rtc_cm:ownedBy"]["rdf:resource"]) {
                    ...
                    if (-1 != lastSlash) {
                        ...
                        statusLine += '_Owned by *' + decodeURIComponent(userName) + '*_. ';
                    }
                }
                ...
                if (wi["dc:creator"] && wi["dc:creator"]["rdf:resource"]) {
                    ...
                    if (-1 != lastSlash) {
                        ...
                        statusLine += '_Created by *' + unescape(userName) + '*_. ';
                    }
                }
...
sess.on(/wi\s*(\d+)/i, handleWorkItemMatch);
sess.on(/defect\s*(\d+)/i, handleWorkItemMatch);
sess.on(/opened\s*(\d+)/i, handleWorkItemMatch);
...
.then(function (success) {
    if (ERR_CHANNEL) {
        sess.sendMessage({
                    channel: ERR_CHANNEL,
                    text: pkg.name + " (" + pkg.version + HYPHEN + BUILD + ") started." 
        });
    }
})
.fail(function (err) {
